...
establish_session() {
	...
	grep -E 'TS016e21d6' | \
	sed "s/.*TS016e21d6\t//g" | \
	xargs -I %arg%  echo "{ \"TS016e21d6\" : \"%arg%\" }" > SessionInfoTmp01.json && \
...
send_quizz_answer() {
    ...
    "https://www.retoactinver.com/reto/app/quiz/contestarQuiz?cveUsuario=osvaldo.hdz.m@outlook.com&cx_tokenSesionApl=$(jq -r ".tokenSession" SessionInfo.json)&cx_token_app=$(jq -r ".tokenApp" SessionInfo.json)&idRespuesta=335&tokenApp=$(jq -r ".tokenApp" SessionInfo.json)&tokenSession=$(jq -r ".tokenSession" SessionInfo.json)"
}
...
help() {
   # Display Help
   echo
   echo "Syntax: SolveDailyQuizz.sh [-h]"
   echo "Options:"
...
echo "\n\nGetting login parameters in SessionInfo.json..."
establish_session
cat SessionInfo.json
echo "\n\nRecovering session for next operation and updating session SessionInfo.json..."
recover_session
echo "\n\nGetting daily quizz..."
get_daily_quizz
